...
export class UserController {
  ...
  async createUser(@Body() dto: CreateUserByAdminDto) {
    // у UserService треба написати метод createByAdmin,
    // який захешує пароль та ПІН перед збереженням
    const newUser = await this.userService.createByAdmin(dto);
  ...
  public async remove(
    ...
    if (user.role !== Role.ADMIN && id !== user.id) {
      throw new ForbiddenException({
        ...
        errors: {
          server: [
            USER_CONTROLLER_MESSAGES.REMOVE.USER_IS_FORBIDDEN_TO_REMOVE.EN,
          ],
        },
      });
    }
